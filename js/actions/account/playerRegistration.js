...
export const createPlayerProfile = () =>
  async ( dispatch, getState ) => {
    ...
    try {
      dev( "Attempting to fetch profile for current set of IDs..." );
      profile = ( await API.playerRegistration.getPlayerProfileForDivision( ids ) )[ 0 ];
      dev( "Profile found. ", profile );

    } catch ( error ) {
      dev( error.status );
      dev( "Profile with current IDs not found, creating a new one..." );
      profile = await API.playerRegistration.createPlayerProfile( ids );
      dev( "New profile created for current set of IDs. ", profile );
...
export const prepareCart = () =>
  async ( dispatch, getState ) => {
    ...
    dev( "Cart updated with registration. ", updatedCart );
